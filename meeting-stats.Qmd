...
# check if there are any large discrepancies between the time stamp and time entered
quality_check <- signin |>
  rename(manual = `Column 8`) |>
  mutate(
    timestamp_hour = hour(Timestamp),
    manual_time = hms(manual),
    manual_hour = if_else(
      stringr::str_detect(manual, "AM") | (hour(manual_time) == 12),
      hour(manual_time),
      hour(manual_time) + 12
    )
  ) |>
  select(manual_hour, timestamp_hour) |>
  filter(manual_hour != timestamp_hour)
```

```{r signout_analysis}
signin_only <- signin_clean |>
  group_by(member, date) |>
  count()

no_signout <- signin_only |>
  filter(n == 1) |>
  nrow()
...
-   `r no_signout` of `r NROW(signin_only)` (`r round((no_signout / NROW(signin_only)) * 100, 2)` %) sign ins did not have a sign out
...
doy_plot <- ggplot(people_per_day, aes(x = date, y = n, fill = day_of_week)) +
  ...
  scale_fill_manual(values = palette("Tableau 10")[2:8])
...
member_time <- signin_clean |>
  ...
  filter(duration > 10)
# removes any short entries (manualy entered)
```

#### Studio usage over days of the week
-  Total number of sign ins `r NROW(member_time)` (removed any data that was outside of the studio open times)
-   Only captures people using the sign in form
-   `r NROW(member_usage)`/ `r NROW(membership)` (`r round((NROW(member_usage)/NROW(membership))*100, 2)` %) total members has been using the sign in form or coming into the studio
...
```{r}
time_data <- member_time |>
  count(hour, date, month_year) |>
  arrange(date, month_year)
...
time_plot <- time_data |>
  ...
  facet_wrap(
    ~ factor(
      month_year,
      levels = purrr::map(2025:year(Sys.Date()), ~ paste(.x, month.abb)) |>
        unlist()
    )
  ) +
...
#### Total sign ins per month
```{r}
member_time |>
  mutate(year = year(`Sign In`)) |>
  count(year, month) |>
  datatable()
```
...
-   Members that did not sign in and out are excluded
-   Assuming the timestamp was sign in sign out time
-   Filtered out entries where there are multiple entries (multiple sign ins in one day)
-   Average member use time is `r mean(member_time$duration) / 60` hours
-   This is constrained by when classes in session and studio availability
...
# Usage Duration by month
# member_time |>
#   group_by(day_of_week, month) |>
#   summarise(average_studio_time = mean(duration)) |>
#   ungroup()|>
#   pivot_wider(names_from = month, values_from = average_studio_time) |>
#   mutate(day_of_week = wday(day_of_week, label = TRUE)) |>
#   datatable()
```